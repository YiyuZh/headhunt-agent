...
如果你是在 Windows 本机调试，就进入 `D:\apps\headhunt-agent`。
...
- `--generate-local-secrets` 会自动替换 `POSTGRES_PASSWORD`、`INTERNAL_ADMIN_API_KEY` 和 `MODEL_SECRET_ENCRYPTION_KEY` 的本地占位值；不要把这些值发到聊天或提交到 GitHub。

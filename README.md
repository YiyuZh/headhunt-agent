...
| Docker Compose | 一条命令启动 API、worker 和 PostgreSQL/pgvector；目标 HTTPS 入口由 `hiremate-caddy` 统一网关承载 |
...
当前服务器实际 HTTPS 状态要分清：已知只有 `ttcintern` 站点完成 HTTPS 配置；`headhunt.zenithy.art` 需要完成统一 Caddy 站点块、DNS、Caddy reload 和 `curl -f https://headhunt.zenithy.art/health` 验证后，才能用于飞书回调。
...
- `PIP_INDEX_URL` 默认使用清华 PyPI 镜像，加速国内服务器的 `docker compose up -d --build`；海外服务器可改回 `https://pypi.org/simple`。
- 如果报 `registry-1.docker.io/v2/` 超时，先修 Docker Hub 拉镜像网络或配置 Docker daemon 的 `registry-mirrors`；这和 `PIP_INDEX_URL` 不是同一层问题。
- `EMBEDDING_API_KEY` 可以先留空；只有需要服务器直接跑 OpenAI embedding fallback 时才填 OpenAI API Key。
- 飞书后台的 `Verification Token` / `Encrypt Key` 在“事件与回调 -> 加密策略”里；事件订阅要选“将事件发送至开发者服务器”，事件 URL 填 `https://headhunt.zenithy.art/feishu/events`，卡片回调填 `https://headhunt.zenithy.art/feishu/card-actions`。完整点击步骤见 [飞书接入操作手册](docs/manual/飞书接入操作手册.md)。
- 第一阶段只要求 `FEISHU_APP_ID`、`FEISHU_APP_SECRET`、`FEISHU_VERIFICATION_TOKEN`、`FEISHU_ENCRYPT_KEY` 和 `FEISHU_DEFAULT_CHAT_ID`；`FEISHU_BITABLE_*` 是第二阶段多维表格同步项，可以先留空。
...
这一步不会输出密钥值；它会检查 `.env` 是否存在、关键变量是否仍是空值或 `change-this-*` 占位符、Docker 是否在 PATH 上，以及 `docker compose` 是否可用。先填完第一阶段飞书真实值再跑 strict；如果仍报 `feishu_required still placeholder`，先回到飞书手册补齐 `.env`。Bitable 未填只会提示 `Optional Bitable business sync is not ready`，不阻断启动。
...
统一网关模式下不会启动本项目自带 `lietou-caddy`。HeadHunt 的目标公网 HTTPS 入口在 `/opt/apps/hiremate` 的 `hiremate-caddy`，它会通过 `shared_gateway` 反代到 `lietou-api:8000`。在统一 Caddy 站点块未部署并验证前，不能把 `headhunt.zenithy.art` 写成已配置 HTTPS。
...
生产使用前，请按手册完成飞书、数据库、LLM 和 HTTPS 回调联调；Bitable 属于第二阶段业务同步联调。
...
`lietou-config-check --strict` 如果只因为 `feishu_bitable` 返回 `degraded`，不阻断第一阶段飞书联调；Bitable 是第二阶段业务同步。
...
Edit `.env` and fill the first-stage Feishu values. Bitable values are only needed for second-stage business sync. Local runtime secrets are generated by the command above:

...
> 状态更新：本文件是 Feishu First 第一版主链路工程设计。飞书事件、飞书交互卡片、飞书 War Room 群消息和 Bitable 同步是第一版普通用户入口；Discord 仅作为后续 optional adapter。
...
审批后 Bitable outbox 排队，以及内部人工审批 API；真实飞书联调未执行。当前实现作为第一版主链路基础，已补齐 TaskIntakeParser 基础、任务确认卡排队和 Feishu BYOK 配置卡表单提交/保存后 setup 卡更新；BYOK 提交会校验操作者与原任务 owner 的同一 ID namespace，且自定义 base_url 在 callback 与真实出站请求前均执行 HTTPS/DNS/private-IP 安全校验；后续要补齐真实飞书后台、Bitable 权限、真实卡片发送/提交/更新、edit 表单、War Room 和 graph 端到端联调。
...
- 飞书回调层只负责快 ACK / toast；完整 AI 工作流必须异步跑。
...
- 飞书 API 调用统一经过 `FeishuAuthProvider`、`FeishuGateway` 和 `FeishuBitableGateway`，不得绕过 `ChannelGateway`、outbox、HumanApproval 或幂等记录。
...
- War Room 卡片可以在任务授权后自动发送；业务写入、报告发布、Bitable 同步、飞书任务创建、外部触达和推荐结论必须人工确认。
...
- `/feishu/events` 和 `/feishu/card-actions` 都按 Encrypt Key 主路径处理：先用 raw body 校验签名，再解密 `encrypt` payload。
- 卡片回调只支持新版 `card.action.trigger`；旧版消息卡片请求网址 / `card.action.trigger_v1` 不作为第一版兼容目标，避免双回调和旧 SHA1 签名误配。

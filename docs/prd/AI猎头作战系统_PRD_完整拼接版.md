...
-> Discord interaction 快 ACK / defer
...
入口层：Discord slash command / button / select / modal、本地 API、后续可选 Hermes/MCP 调用公开入口。
工作台层：Discord War Room 展示结构化过程、证据、评论、修改、批准和复盘。
```

目标系统一步到位设计；工程实现可以按波次拆分，但第一版主链路必须直接接入真实 Postgres + pgvector、真实 Discord interactions / cards / modals / threads。飞书 / Bitable 作为后续可选 adapter，不进入第一版主验收路径。mock / fake gateway 只用于单元测试、CI 和失败隔离，不作为第一版主运行路径。
...
13_Discord工作台与AgentSOP交付_PRD.md
```
...
- Discord 是第一版猎头工作台；飞书 / Bitable 是后续可选 adapter。
...
- Discord War Room 能看到每个 Agent 的结构化过程、证据、工具轨迹、token 消耗、ReviewGate 结果和可编辑建议。
...
| 工部 | `DataAutomationAgent` | 从数据导入、Discord 工作台、数据库、自动化可行性角度审查需求 |
...
## 6. Discord / ChannelGateway 可视化规则

会审层必须把结构化过程同步到 Discord War Room；Feishu 仅作为 deferred adapter：
...
- Discord、本地内部 API、未来 Hermes/MCP 入口都必须先进入会审层。
...
- 部门 Agent 不能直接互相调用。
- 部门 Agent 不能直接访问数据库、公网、Discord、飞书或模型 Key。
...
- Discord War Room 能展示会审模式、模式原因和本次实际运行 Agent 的结构化卡片。
# 模块 03：数据体系与案例导入 PRD
...
本模块定义 Agent 协作操作系统。目标不是让 Agent 自由聊天，而是让每个 Agent 在授权范围内读取结构化输入、等待上游 artifact、使用工具和 SOP、输出结构化 artifact，并让用户在 Discord War Room 里看到可审计、可修改、可复盘的过程。
...
user correction：用户在 Discord modal/edit/reject 中修正过的内容。
...
Discord edit、reject 原因、人工改稿可生成 UserCorrectionMemoryProposal。
错误记忆必须可撤销、可标记失效。
...
## 10. Discord War Room 可观测

不展示原始 chain-of-thought，展示可审计工作过程。

一个任务对应一个 Discord thread / message 组。每个 Agent 输出一张结构化卡片：
...
edit：打开 Discord modal，形成 edited_payload 或 UserCorrection artifact。
reject：停止或回退，记录原因。
...
Discord interaction 也必须可观测：

```text
/discord/interactions 快速 ACK 记录 DiscordInteractionLog。
...
Discord 默认 standard 模式，不发送完整 debug artifact。
...
Discord War Room 能看到每个 Agent 的结构化过程、ReviewGate 结果和 token 消耗。
Discord War Room 和 AgentRuns 能展示本次注入了哪些 memory_refs / sop_refs、命中原因和 token 估算，但不展示完整隐私内容或原始 prompt。
